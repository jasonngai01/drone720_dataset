...
#116 & 118
#print scores_output_index should 118 if output_details[1]['index']
...
def sd_callback(rec, frames, time, status):
    ...
    rec_1 = tf.expand_dims(rec , 0) # change the waveform.shape from (15600,) to (1,15600) to fit the model maker 
    #print(rec.shape)
    #print(rec1.shape)
    ...
    top_score_y = float(top_score_0)
    
    ###############################################################################################################################
    
    prediction = np.mean(scores_0, axis=0)
    ...
    if uav_prediction == 1:
        print('{:.3f}%      UAV detected'.format(top_score))
    ...
    #print(uav_prediction) # show 0 or 1 , uav detected or not
    #print(top_score)
    #print(inferred_class)    
    #print(top_score_y)
    #print('{:.3f} % {:12s} detected'.format(top_score_y,inferred_class))
    ...
    n = 0 
    for i in top3_i:
        #print(prediction[i]) #0.26171875
        #print(inferred_class_1[n])
